...
public class PlayerLedgeClimbState : PlayerState
{
    ...
    public override void Enter()
    {
        ...
        startPos.Set(cornerPos.x - (core.movement.faceingDirection * playerData.startOffset.x), cornerPos.y - playerData.startOffset.y);
        stopPos.Set(cornerPos.x + (core.movement.faceingDirection * playerData.stopOffset.x), cornerPos.y + playerData.stopOffset.y);
    ...
    public override void LogicUpdate()
    {
        ...
        else
        {
            ...
            if (inputX == core.movement.faceingDirection && isHanging && !isClimbing)
            {
                CheckForSpace();
                isClimbing = true;
    ...
    {
        isTouchingCeiling = Physics2D.Raycast(cornerPos + (Vector2.up * 0.015f) + (Vector2.right * core.movement.faceingDirection * 0.015f), Vector2.up, playerData.standColliderHeight, core.collisionSenses.WhatIsGround);
    ...
    {
        RaycastHit2D xHit = Physics2D.Raycast(core.collisionSenses.WallCheck.position, Vector2.right * core.movement.faceingDirection, core.collisionSenses.WallCheckDistance, core.collisionSenses.WhatIsGround);
        ...
        workspace.Set(core.collisionSenses.WallCheck.position.x + (xDist * core.movement.faceingDirection), core.collisionSenses.LedgeCheck.position.y - yDist);
